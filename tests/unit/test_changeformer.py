...
from geoai.models.transformers.changeformer import ChangeFormer, ChangeFormerArch
from geoai.models.transformers.base import TransformerBaseWrapper

class TestChangeFormerWrapper:
    """Unit tests for the ChangeFormer wrapper class and registered pipeline operations."""

    def test_registry_lookup(self):
        """Verify that ChangeFormer is correctly registered and instantiable from registry."""
        model = get_experiment_model("changeformer")
        assert isinstance(model, ChangeFormer)
    ...
    def test_wrapper_initialization(self):
        """Verify ChangeFormer wrapper initialization sets up custom architectures and default values."""
        model = ChangeFormer()
        assert model.model_id == "changeformer"
        assert model.architecture_class == ChangeFormerArch
        assert model.model is None

    def test_forward_pass_and_inference(self):
        """Test the forward pass of ChangeFormerArch and mock inference from the wrapper."""
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model_arch = ChangeFormerArch(in_channels=18, out_channels=2).to(device)
        model_arch.eval()
        ...
        wrapper = ChangeFormer()
        wrapper.model = ChangeFormerArch(in_channels=18, out_channels=2)
    ...
    def test_explainability_hooks(self):
        ...
        # 3 stages * 32 projected channels = 96 channels
        assert token_emb.shape == (2, 96, 15, 15)
    ...
    def test_training_lifecycle(self):
        """Test one epoch of training of ChangeFormer wrapper on synthetic features."""
        wrapper = ChangeFormer()
        wrapper.device = torch.device("cpu")
        wrapper.hyperparams = {
            "batch_size": 2,
