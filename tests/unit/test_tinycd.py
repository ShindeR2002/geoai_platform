...
from geoai.models.transformers.tinycd import TinyCD, TinyCDArch
from geoai.models.transformers.base import TransformerBaseWrapper

class TestTinyCDWrapper:
    """Unit tests for the TinyCD wrapper class and registered pipeline operations."""

    def test_registry_lookup(self):
        """Verify that TinyCD is correctly registered and instantiable from registry."""
        model = get_experiment_model("tinycd")
    ...
    def test_wrapper_initialization(self):
        """Verify TinyCD wrapper initialization sets up custom architectures and default values."""
        model = TinyCD()
        assert model.model_id == "tinycd"
        assert model.architecture_class == TinyCDArch
        assert model.model is None

    def test_forward_pass_and_inference(self):
        """Test the forward pass of TinyCDArch and mock inference from the wrapper."""
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model_arch = TinyCDArch(in_channels=18, out_channels=2).to(device)
        model_arch.eval()
        ...
        wrapper = TinyCD()
        wrapper.model = TinyCDArch(in_channels=18, out_channels=2)
