...
def main():
    config_path = root_dir / "configs" / "experiments" / "changeformer_quick_profile.yaml"
    runner = ExperimentRunner(outputs_dir=str(root_dir / "outputs" / "experiments"), configs_dir=str(root_dir / "configs"))
    
    print("Starting ChangeFormer Quick Profile run...")
    try:
        ...
        if experiment.get_state() == "Completed":
            print("Success! ChangeFormer pipeline ran successfully.")
            sys.exit(0)
        else:
            print("Failure: ChangeFormer pipeline execution failed.")
            sys.exit(1)
