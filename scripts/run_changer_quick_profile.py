...
def main():
    config_path = root_dir / "configs" / "experiments" / "changer_quick_profile.yaml"
    runner = ExperimentRunner(outputs_dir=str(root_dir / "outputs" / "experiments"), configs_dir=str(root_dir / "configs"))
    
    print("Starting Changer Quick Profile run...")
    try:
        ...
        if experiment.get_state() == "Completed":
            print("Success! Changer pipeline ran successfully.")
            sys.exit(0)
        else:
            print("Failure: Changer pipeline execution failed.")
            sys.exit(1)
